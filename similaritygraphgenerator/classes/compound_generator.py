...
class CompoundGenerator:
    ...
    def generate_compound(self, type_rules):
        ...
        num_additions = self.rng.integers(
            *type_rules["additions"]
        ...
        sequence = np.concatenate(
            (start_sequence, additions)
        )  # Combine the start sequence with the generated bl
        return Compound(sequence, type_rules["name"])
...
class Compound:
    ...
    def __repr__(self):
        return "Type: " + self.compound_type + " | Sequence: " + self.sequence
